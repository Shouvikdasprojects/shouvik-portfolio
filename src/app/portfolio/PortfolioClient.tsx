        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 space-y-6">
              <TiltCard className="p-6 bg-[#0b0814]/50 border-white/5 shadow-2xl relative overflow-hidden group rounded-2xl">
                {/* Visual Accent */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-primary/10 rounded-full filter blur-xl pointer-events-none" />

                <div className="relative w-full aspect-square rounded-2xl overflow-hidden mb-6 bg-slate-900 border border-white/10">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <span className="text-xs font-mono font-bold text-primary">{personalInfo.role}</span>
                  </div>
                  ...
                  <div className="pt-4 border-t border-white/5 flex flex-wrap gap-4 items-center justify-between">
                    <span className="text-xs text-gray-500 font-mono flex items-center gap-1.5">
                      <MapPin size={12} className="text-primary" /> {personalInfo.aboutDetails.location}
                    </span>
              ...
              {/* Email details card */}
              <GlassCard className="p-6 bg-[#0b0814]/50 border-white/5 relative overflow-hidden rounded-2xl">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <a 
                      ...
                      className="text-white hover:text-primary font-bold text-sm transition-colors break-all"
                    >
                      {personalInfo.email}
                    </a>
                  </div>
                  <a 
                    href={`mailto:${personalInfo.email}`} 
                    className="p-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-primary hover:bg-primary/10 text-white transition-all"
                    title="Send Email"
                  >
            ...
            <div className="lg:col-span-7 space-y-8">
              ...
              {/* Full Details & Skills */}
              <GlassCard className="p-6 md:p-8 bg-[#0b0814]/40 border-white/5 space-y-6 rounded-2xl">
                <div>
                  <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-1">
                    <Award size={18} className="text-primary" /> Full Biographical Details
                  </h3>
                ...
                <div className="space-y-3">
                  <span className="text-[10px] font-bold font-mono tracking-widest text-gray-500 uppercase flex items-center gap-1.5">
                    <Heart size={10} className="text-primary" /> My Hobbies & Activities
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {personalInfo.aboutDetails.hobbies.map((hobby, index) => (
                      <div key={index} className="flex items-center gap-2 px-3.5 py-2 bg-white/[0.02] border border-white/5 rounded-xl text-xs text-gray-300">
                ...
                <div className="space-y-3 pt-2">
                  <span className="text-[10px] font-bold font-mono tracking-widest text-gray-500 uppercase flex items-center gap-1.5">
                    <Sparkles size={10} className="text-secondary" /> Primary Skills Spectrum
                  </span>
                  <div className="flex flex-wrap gap-2">
                    {personalInfo.aboutDetails.skills.map((skill, index) => (
                      <span key={index} className="text-xs font-semibold font-mono bg-white/5 border border-white/10 text-gray-200 px-3 py-1.5 rounded-xl shadow-sm">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {/* Social Media accounts list (specifically filtered to show 5) */}
              <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-1">
                    <MessageSquare size={18} className="text-primary" /> Connect On Social Media
                  </h3>
                  <div className="w-12 h-[2px] bg-primary rounded-full" />
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {portfolioSocials.map((social) => (
                    <motion.div 
                      key={social.name}
                      whileHover={{ y: -2 }}
                      className="group flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#0b0814]/40 border border-white/5 p-6 rounded-2xl hover:border-white/10 transition-all duration-300"
                    >
                      <div className="flex items-center gap-4">
                      ...
                      <a 
                        ...
                        target="_blank" 
                        rel="noreferrer"
                        className="w-full sm:w-auto py-2.5 px-5 text-center text-xs font-bold rounded-xl border border-white/10 hover:border-primary bg-white/5 hover:bg-primary text-white flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
                      >

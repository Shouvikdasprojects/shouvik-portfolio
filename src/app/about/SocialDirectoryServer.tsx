  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      {dynamicYoutube.map((channel, idx) => (
        <GlassCard key={channel.name} className="flex flex-col bg-[#0b0814]/40 border-white/5 p-6 hover:scale-[1.02] transition-transform rounded-2xl">
          ...
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            className="w-full mt-6 py-2.5 text-center text-xs font-bold bg-red-600 hover:bg-red-700 text-white rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-md"
          >
      ...
      {dynamicSocials.filter(s => s.type === 'Instagram').map((social) => (
        <GlassCard key={social.name} className="flex flex-col bg-[#0b0814]/40 border-white/5 p-6 hover:scale-[1.02] transition-transform rounded-2xl">
          <div className="flex items-center gap-4 justify-between mb-4">
            <div className="flex items-center gap-4">
              <div className="photo-glow-border w-14 h-14 overflow-hidden relative rounded-full aspect-square bg-slate-900 border border-white/10 shrink-0">
                <SafeImage 
              ...
              <div className="text-left">
                ...
                <span className="text-[9px] font-mono text-primary bg-primary/10 border border-primary/20 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider block w-max mt-1">
                  Instagram
                </span>
              </div>
            </div>
          ...
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            className="w-full mt-6 py-2.5 text-center text-xs font-bold rounded-xl border border-white/10 hover:border-primary bg-white/5 hover:bg-primary text-white flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
          >
      ...
      {dynamicSocials.filter(s => s.type === 'Facebook').map((social) => (
        <GlassCard key={social.name} className="flex flex-col bg-[#0b0814]/40 border-white/5 p-6 hover:scale-[1.02] transition-transform rounded-2xl">
          ...
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            className="w-full mt-6 py-2.5 text-center text-xs font-bold rounded-xl border border-white/10 hover:border-blue-500 bg-white/5 hover:bg-blue-600 text-white flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
          >
      ...
      {dynamicSocials.filter(s => s.type === 'HeyLink').map((social) => (
        <GlassCard key={social.name} className="flex flex-col bg-[#0b0814]/40 border-white/5 p-6 hover:scale-[1.02] transition-transform rounded-2xl">
          ...
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            className="w-full mt-6 py-2.5 text-center text-xs font-bold rounded-xl border border-white/10 hover:border-green-500 bg-white/5 hover:bg-green-600 text-white flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
          >

              ...
              <TiltCard key={idx} className="flex flex-col h-full bg-[#0b0814]/40 border-white/5 p-6 rounded-2xl">
                ...
                <div className="relative w-full h-48 rounded-xl overflow-hidden mb-6 bg-slate-900 border border-white/5">
                  ...
                  <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md text-white text-[9px] font-bold font-mono px-2.5 py-1 rounded flex items-center gap-1">
                    {post.type === 'Video' || post.type === 'Vlog' ? <Film size={10} className="text-red-500" /> : <ImageIcon size={10} className="text-primary" />}
                    {post.type.toUpperCase()}
                  </div>

                  {/* Channel Tag */}
                  <div className="absolute bottom-3 right-3 bg-primary/90 text-white font-mono text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                    {post.source}
                  </div>
                </div>
